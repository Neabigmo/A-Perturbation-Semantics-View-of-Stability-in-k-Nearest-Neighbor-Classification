...
the paper.  The supplementary material contains a somewhat richer figure set
under \texttt{outputs/figures/}, including intermediate mechanism schematics and
witness renderings.  These assets are part of the reproducibility record.
...
\begin{figure}[h]
    ...
    \caption{Two cases for deletion.  Case A (left): deleting an occurrence
    outside the ordered top-$k$ neighborhood is inert; the top-$k$ set and
    margin are unchanged.  Case B (right): deleting an occurrence inside the
    top-$k$ set may promote the rank-$(k+1)$ occurrence into the vote, changing
    the margin.}
